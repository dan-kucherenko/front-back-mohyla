...
function AddEmployee() {
    ...
    const btnClick = () => {
        ...
        $.ajax({
            method: "POST",
            url: "http://localhost:4567/graphql", // Ensure this is the correct GraphQL endpoint
            contentType: "application/json",
            data: JSON.stringify(graphqlQuery),
            success: function (response) {
                if(response.data && response.data.addEmployee) {
                    alert("Employee has been added");
                    clearInputs();
                } else {
                    alert("Failed to add employee. Check the data you have entered.");
                }
            },
